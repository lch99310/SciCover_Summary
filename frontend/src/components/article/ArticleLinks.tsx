 ...
 */
function getPreprintLabel(url: string): { zh: string; en: string } {
  if (url.includes('arxiv.org')) return { zh: 'arXiv 預印本', en: 'arXiv Preprint' };
  if (url.includes('biorxiv.org')) return { zh: 'bioRxiv 預印本', en: 'bioRxiv Preprint' };
  if (url.includes('ssrn.com')) return { zh: 'SSRN 預印本', en: 'SSRN Preprint' };
  if (url.includes('socarxiv') || url.includes('socopen.org')) return { zh: 'SocArXiv 預印本', en: 'SocArXiv Preprint' };
  if (url.includes('osf.io')) return { zh: 'OSF 預印本', en: 'OSF Preprint' };
  return { zh: '預印本', en: 'Preprint' };
}
...
export function ArticleLinksSection({ keyArticle, links }: ArticleLinksProps) {
  ...
  const preprintLabel = links.preprint ? getPreprintLabel(links.preprint) : { zh: '預印本', en: 'Preprint' };

  return (
    <div className="article-links">
      <div className="article-links__paper">
        <h3 className="article-links__heading">重點研究論文 / Key Research Article</h3>
      ...
      <div className="article-links__buttons">
        ...
        {links.preprint ? (
          ...
          >
            ...
            {preprintLabel.zh} / {preprintLabel.en}
          </a>
        ) : (
          <button
            type="button"
            ...
            title="此論文目前沒有公開的預印本連結 / No preprint available for this article"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
              <polyline points="14 2 14 8 20 8" />
      ...
      {showToast && (
        <div className="article-links__toast">
          此論文目前沒有公開的預印本連結 / No preprint available
        </div>
      )}
    </div>
  );
